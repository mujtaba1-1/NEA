...
class FaceAttendance:
    def __init__(self, window, window_title, connection):
        ...
        self.video_source = 2  # Use the default webcam (change if needed)
    ...
    def recognise_faces(self, frame, x, y, x1, y1):
        ...
        self.temp_validation_faces.append((class_indices[0], time.time()))

        face_valid = self.validation()
    ...
    def validation(self):
        ...
        for face in self.temp_validation_faces:
            if current_time - face[1] <= self.validation_duration:
                validation_results.append(face[0])

        self.update_output(f"{len(validation_results)}")
